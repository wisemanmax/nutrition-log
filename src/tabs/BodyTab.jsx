...
function LogBodySheet({ existing, units, onSave, onClose }) {
  ...
  return (
    <Sheet title={existing ? 'Edit Entry' : 'Log Body'} onClose={onClose}
      ...
      <div style={{ fontSize: 11, fontWeight: 700, color: V.text3, textTransform: 'uppercase', letterSpacing: '.08em', marginBottom: 8, marginTop: 4 }}>Measurements ({dU})</div>
  ...
  return (
    <div style={{ marginTop: 8 }}>
      ...
      {photoUrl ? (
        ...
        <button onClick={capturePhoto} style={{ width: '100%', minHeight: 120, border: `2px dashed ${V.cardBorder}`, borderRadius: 12, background: 'transparent', color: V.text3, cursor: 'pointer', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 6 }}>
...
export function BodyTab({ s, d }) {
  ...
  const prevWaist = body.find(b => b.date !== today() && b.waist != null)?.waist;
  ...
  return (
    <div className="fade-up" style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        ...
        <Btn onClick={() => setShowLog(true)}>{Icons.plus({ size: 14, color: '#060a0e' })} Log Today</Btn>
      </div>
      ...
      {todayEntry ? (
        <>
          ...
          {/* BMI estimate if height available */}
      ...
      {body.length > 0 && (
        <Card style={{ padding: 12 }}>
          <div style={{ fontSize: 11, fontWeight: 700, color: V.text, marginBottom: 10 }}>History</div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 0 }}>
            {body.slice(0, 10).map(entry => (
              <div key={entry.id} style={{ display: 'flex', alignItems: 'center', padding: '8px 0', borderBottom: `1px solid rgba(255,255,255,0.03)` }}>
                ...
                <div style={{ display: 'flex', gap: 4 }}>
                  <button onClick={() => setEditEntry(entry)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: V.text3, padding: 4, fontSize: 13 }} aria-label="Edit">✏️</button>
                  <button onClick={() => deleteEntry(entry.id)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: V.danger, padding: 4, fontSize: 13 }} aria-label="Delete">🗑</button>
                </div>
              </div>
            ))}
          </div>
